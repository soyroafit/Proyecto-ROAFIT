...
export default function ClientHome({ session, onOpenDay }) {
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {days.map((d) => (
              <div
                key={d.id}
                onClick={() => onOpenDay(d.id, d.label)}
                style={{ background: '#1B1B1F', border: '1px solid #2A2A2F', borderRadius: 12, padding: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
              >
